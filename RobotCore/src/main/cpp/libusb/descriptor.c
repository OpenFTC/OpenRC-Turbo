...
{
    if (!dev->device_descriptor_valid) {
        int r, host_endian = 0;
        memset(&dev->device_descriptor, 0, sizeof(dev->device_descriptor));

        r = usbi_backend.get_device_descriptor(dev, (unsigned char *) &dev->device_descriptor, &host_endian);
        if (r < 0)
            return r;

        dev->device_descriptor_valid = TRUE;
        if (!host_endian) {
 ...
 */
int API_EXPORTED libusb_get_device_descriptor(libusb_device *dev, struct libusb_device_descriptor *desc)
{
	memcpy((unsigned char *) desc, (unsigned char *) &dev->device_descriptor, sizeof(dev->device_descriptor));
	return dev->device_descriptor_valid ? LIBUSB_SUCCESS : originate_err(LIBUSB_ERROR_NOT_FOUND);
}

int LIBUSB_CALL libusb_get_serial_number(libusb_device* device, LPSTR* psz)
{
	int r = LIBUSB_SUCCESS;
	if (usbi_backend.get_serial_number) {
		r = usbi_backend.get_serial_number(device, psz);
	} else {
		r = originate_err(LIBUSB_ERROR_OTHER);
	    *psz = NULL;
	}
	return r;
}
 ...
 */
int API_EXPORTED libusb_get_active_config_descriptor(libusb_device *dev, struct libusb_config_descriptor **config)
{
	...
	if (r < LIBUSB_DT_CONFIG_SIZE) {
		usbi_err(dev->ctx, "short config descriptor read %d/%d", r, LIBUSB_DT_CONFIG_SIZE);
		return originate_err(LIBUSB_ERROR_IO);
	}
 ...
 */
int API_EXPORTED libusb_get_config_descriptor(libusb_device *dev, uint8_t config_index, struct libusb_config_descriptor **config)
{
	struct libusb_config_descriptor _config;
	unsigned char tmp[LIBUSB_DT_CONFIG_SIZE];
	unsigned char *buf = NULL;
 ...
 */
int usbi_get_config_index_by_value(struct libusb_device *dev, uint8_t bConfigurationValue, int *idx)
{
	uint8_t i;
