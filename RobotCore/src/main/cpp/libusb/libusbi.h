...
{
    if (entry->next) entry->next->prev = entry->prev;   // -rga make idempotent
	if (entry->prev) entry->prev->next = entry->next;   // -rga make idempotent
...
#define _usbi_log_always(ctx, level, ...) (usbi_log(ctx, level, __FUNCTION__, ##__VA_ARGS__), 0)
#define usbi_err_always(ctx, ...)		  _usbi_log_always(ctx, LIBUSB_LOG_LEVEL_ERROR, ##__VA_ARGS__)
#define usbi_warn_always(ctx, ...)		  _usbi_log_always(ctx, LIBUSB_LOG_LEVEL_WARNING, ##__VA_ARGS__)
#define usbi_info_always(ctx, ...)		  _usbi_log_always(ctx, LIBUSB_LOG_LEVEL_INFO, ##__VA_ARGS__)
#define usbi_dbg_always(...)		      _usbi_log_always(NULL, LIBUSB_LOG_LEVEL_DEBUG, ##__VA_ARGS__)

#if ENABLE_ERROR_ORIGINATION_LOGGING
    #define originate_err(rc) (_originate_err((rc), __FILE__, __LINE__))
	inline static int _originate_err(int rc, LPCSTR file, int line) // don't know why we need the 'static'...silly C...:-)
		{
		if (rc != 0)
			{
			usbi_dbg_always("[%s:%d] originating err: %d(%s)", libusbBaseNameOfFile(file), line, (rc), libusb_error_name(rc));
			}
		return rc;
		}
#else
    #define originate_err(rc) (rc)
#endif
...
//#define usbi_err(ctx, ...) do { (void)ctx; } while (0) // always log errors to help interpret logs
#define usbi_err(ctx, ...)	usbi_err_always(ctx, ##__VA_ARGS__)
...
struct libusb_context {
	...
	/* -rga Linux: location of root of USB file system. */
	LPCSTR szUsbfsRoot;
	int weOwnSzUsbfsRoot; // whether we need to free szUsbfsRoot on exit
...
void _lockEventData(libusb_context*, LPCSTR, int);
void _unlockEventData(libusb_context*, LPCSTR, int);

#define lockEventData(ctx)		_lockEventData(ctx, __FUNCTION__, __LINE__)
#define unlockEventData(ctx)	_unlockEventData(ctx, __FUNCTION__, __LINE__)
...
struct libusb_device {
	/* lock protects refcnt & szUsbPath & device_descriptor, everything else is finalized at initialization time */
	usbi_mutex_t lock;
	int refcnt;
	...
	uint8_t bus_number;
	uint8_t port_number;
	LPCSTR szSysfsDir; // http://www.linux-usb.org/FAQ.html#i6
	...
	struct libusb_device_descriptor device_descriptor; // may be unfilled on Nougat and beyond until we open
	int device_descriptor_valid;
	int attached;
...
struct usbi_os_backend {
    ...
    /** Version of init() that allows caller to explicitly indicate the location  
     *  of the USB file system in Linux. Necessary on Android > Android K */
	int (*init)(struct libusb_context *ctx, /*nullable*/ const char* szUsbfs, int buildVersionSDKInt, int forceJavaUsbEnumerationKitKat);
    ...
    int (*open)(struct libusb_device_handle *dev_handle, int fd, LPCSTR szUsbPath);

    // -rga: create a new libusb_device from whole cloth, given its path in the USB file system
	struct libusb_device* (*create)(struct libusb_context *ctx, LPCSTR szUsbPath);
	...
	int (*get_serial_number)(struct libusb_device *device, LPSTR* psz);
