...
static void discovered_devs_free(struct discovered_devs *discdevs)
{
	...
	for (i = 0; i < discdevs->len; i++)
		libusb_unref_device2(discdevs->devices[i], "discovered_devs_free");
	...
	struct discovered_devs *discdevs, struct libusb_device *dev)
{
	...
	if (len < discdevs->capacity) {
		discdevs->devices[len] = libusb_ref_device2(dev, "discovered_devs_append");
		discdevs->len++;
		return discdevs;
	}
...
struct libusb_device *usbi_alloc_device(struct libusb_context *ctx, unsigned long session_id)
{
	...
	int r = usbi_mutex_init(&dev->lock);
	if (r) {
		free(dev);
		return NULL;
	}
	...
	dev->szSysfsDir = NULL;
    dev->device_descriptor_valid = FALSE;
...
{
    if (!dev->attached) { // -rga: make idepotent: in non-hotplug case we seem to connect twice
...
int usbi_sanitize_device(struct libusb_device *dev)
{
	...
	int r = usbi_device_cache_descriptor(dev);
	if (r < 0)
		return LIBUSB_SUCCESS; // ignore error if we don't have descriptors here in native code
...
struct libusb_device *usbi_get_device_by_session_id(struct libusb_context *ctx, unsigned long session_id)
{
	...
	list_for_each_entry(dev, &ctx->usb_devs, list, struct libusb_device) {
		if (dev->session_data == session_id) {
			ret = libusb_ref_device2(dev, "usbi_get_device_by_session_id");
			break;
	    }
    }
	usbi_mutex_unlock(&ctx->usb_devs_lock);
 ...
 */
ssize_t API_EXPORTED libusb_get_device_list_kitkat(libusb_context *ctx, libusb_device ***list)
{
	...
	for (i = 0; i < len; i++) {
		...
		ret[i] = libusb_ref_device2(dev, "libusb_get_device_list");
	}
	*list = ret;
		...
		int unref_devices)
{
	...
	if (unref_devices) {
		...
		while ((dev = list[i++]) != NULL)
			libusb_unref_device2(dev, "libusb_free_device_list");
	}
	free(list);
}
 ...
 */
DEFAULT_VISIBILITY
libusb_device * LIBUSB_CALL libusb_ref_device2(libusb_device *dev, LPCSTR caller)
{
	usbi_mutex_lock(&dev->lock);
	dev->refcnt++;
    usbi_dbg("busnum %d devaddr %d refcnt %d caller %s", dev->bus_number, dev->device_address, dev->refcnt, caller);
	usbi_mutex_unlock(&dev->lock);
	return dev;
}
...
{
    return libusb_ref_device2(dev, NULL);
}
 ...
 */
void API_EXPORTED libusb_unref_device2(libusb_device *dev, LPCSTR caller)
{
	int refcnt;
	...
	if (refcnt == 0) {
		...
		libusb_unref_device2(dev->parent_dev, "parent");
...
{
    libusb_unref_device2(dev, NULL);
}

#if 0 // was written to older version of libusb
/*
...
void usbi_fd_notification(struct libusb_context *ctx) {
	...
	ctx->pollfd_modify++;   // pollfd_modify is now device_close
 ...
 */
int API_EXPORTED libusb_open(libusb_device* dev, libusb_device_handle** dev_handle, int fd, LPCSTR szUsbPath)
{
    *dev_handle = NULL; // initialize result
	if (!isValidFd(fd) || szUsbPath==NULL) {
		return LIBUSB_ERROR_INVALID_PARAM;
	}
	...
	_dev_handle->dev = libusb_ref_device2(dev, "libusb_open");
	_dev_handle->auto_detach_kernel_driver = FALSE;
	_dev_handle->claimed_interfaces = 0;
	memset(&_dev_handle->os_priv, 0, priv_size);

    r = usbi_backend.open(_dev_handle, fd, szUsbPath); // callee will dup the handle and copy the path

	if (r < 0) {
		usbi_dbg("open %d.%d returns %d", dev->bus_number, dev->device_address, r);
		libusb_unref_device2(dev, "libusb_open");
	...
	*dev_handle = _dev_handle;
	...
	/* saki: "At this point, we want to interrupt any existing event handlers so
	 * that they realise the addition of the new device's poll fd. One
	 * example when this is desirable is if the user is running a separate
	 * dedicated libusb events handling thread, which is running with a long
	 * or infinite timeout. We want to interrupt that iteration of the loop,
	 * so that it picks up the new fd, and then continues." */
	// usbi_fd_notification(ctx); // saki
	...
	struct libusb_device_handle *dev_handle)
{
	...
	libusb_unref_device2(dev_handle->dev, "do_close");
	usbi_mutex_destroy(&dev_handle->lock);
	free(dev_handle);
}

void _lockEventData(libusb_context* ctx, LPCSTR caller, int line)
	{
    // if (caller==NULL) caller = "";
	// usbi_dbg_always("%s(%d)...", caller, line);
	usbi_mutex_lock(&ctx->event_data_lock);
	// usbi_dbg_always("...%s(%d)...", caller, line);
	}
void _unlockEventData(libusb_context* ctx, LPCSTR caller, int line)
	{
    // if (caller==NULL) caller = "";
	// usbi_dbg_always("...%s(%d)", caller, line);
...
/** \ingroup libusb_dev
 * Creates a usb device from whole cloth
 * UseCase: Android
 * \param a szUsbPath device path
 * \returns allocate a device from the szUsbPath
 */
DEFAULT_VISIBILITY
libusb_device* LIBUSB_CALL libusb_create(libusb_context *ctx, LPCSTR szUsbPath)
{
	if (usbi_backend.create == NULL) {
		/* Not supported on this platform */
		return NULL;
	}

	return usbi_backend.create(ctx, szUsbPath);
}
		...
		int interface_number)
{
    ...
    // -rga: saki had additional logic here that on LIBUSB_ERROR_BUSY tried to release the intf and then retry the claim
	if (r == 0)
...
int API_EXPORTED libusb_note_claimed_interface(libusb_device_handle *dev_handle,
		int interface_number)
{
	int r = 0;
...
int API_EXPORTED libusb_note_released_interface(libusb_device_handle *dev_handle,
	int interface_number)
{
	int r;
...
{
#if ENABLE_LOGGING && !ENABLE_DEBUG_LOGGING
	USBI_GET_CONTEXT(ctx);
	if (!ctx->debug_fixed) {
 ...
 * \see libusb_contexts
 */
int API_EXPORTED libusb_init(libusb_context** context, LPCSTR szUsbfsRoot, int buildVersionSDKInt, int forceJavaUsbEnumerationKitKat)
{
	...
	ctx->szUsbfsRoot = NULL;
	ctx->weOwnSzUsbfsRoot = FALSE;

	int rcBackend = usbi_backend.init(ctx, szUsbfsRoot, buildVersionSDKInt, forceJavaUsbEnumerationKitKat);
...
err_backend_exit:
	if (rcBackend == 0 && usbi_backend.exit)
...
err_free_ctx:
	...
	list_for_each_entry_safe(dev, next, &ctx->usb_devs, list, struct libusb_device) {
		list_del(&dev->list);
		libusb_unref_device2(dev, "libusb_init");
	}
	usbi_mutex_unlock(&ctx->usb_devs_lock);
	...
	if (ctx->weOwnSzUsbfsRoot) {
		free((void*)ctx->szUsbfsRoot);
	}
	free(ctx);
err_unlock:
	usbi_mutex_static_unlock(&default_context_lock);
	return r!=0 ? r : rcBackend;
}
...
void API_EXPORTED libusb_exit(struct libusb_context *ctx)
{
	...
	if (libusb_has_capability(LIBUSB_CAP_HAS_HOTPLUG)) {
		...
		list_for_each_entry_safe(dev, next, &ctx->usb_devs, list, struct libusb_device) {
			list_del(&dev->list);
			libusb_unref_device2(dev, "libusb_exit");
		}
		usbi_mutex_unlock(&ctx->usb_devs_lock);
	}
