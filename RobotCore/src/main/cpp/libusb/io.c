...
int API_EXPORTED libusb_submit_transfer(struct libusb_transfer *transfer)
{
	...
	if (r == LIBUSB_SUCCESS) {
		...
		libusb_ref_device2(transfer->dev_handle->dev, "transfer");
	} else {
		originate_err(r);
	}
	usbi_mutex_unlock(&itransfer->lock);
	...
	enum libusb_transfer_status status)
{
	...
	libusb_unref_device2(dev_handle->dev, "transfer");
	return r;
}
 ...
 * \ref libusb_mtasync
 *
 * -rga: note: libusb has race issues if libusb_init() fails: libusb_interrupt_event_handler
 * can get called even if that fn returned failure. For now, we ignore, trying not to fail the
...
static int handle_events(struct libusb_context *ctx, struct timeval *tv)
{
	...
	if (ctx->event_flags & USBI_EVENT_POLLFDS_MODIFIED) {
		...
		if (!ctx->pollfds) {
			unlockEventData(ctx);
			r = originate_err(LIBUSB_ERROR_NO_MEM);
			goto done;
		}
	...
	if (r == 0) {
		...
		r = originate_err(LIBUSB_ERROR_INTERRUPTED);
		goto done;
	} else if (r < 0) {
	...
	if (fds[0].revents) {
		...
		while (!list_empty(&hotplug_msgs)) {
			...
			if (LIBUSB_HOTPLUG_EVENT_DEVICE_LEFT == message->event)
				libusb_unref_device2(message->device, "handleEvent");
