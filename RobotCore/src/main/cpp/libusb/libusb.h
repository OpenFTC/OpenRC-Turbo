...
#define LIBUSB_DT_HEADER_SIZE			2 // -rga: added by saki
...
inline static LPCSTR libusbBaseNameOfFile(LPCSTR file)
	{
	if (file != NULL)
		{
...
int LIBUSB_CALL libusb_init(libusb_context** ctx, const char* szUsbfsRoot, int buildVersionSDKInt, int forceJavaUsbEnumerationKitKat);
...
ssize_t LIBUSB_CALL libusb_get_device_list_kitkat(libusb_context *ctx,
        libusb_device ***list);
...
void LIBUSB_CALL libusb_unref_device2(libusb_device *dev, LPCSTR caller);
...
uint8_t LIBUSB_CALL libusb_get_linux_dev_addr(libusb_device* dev);
...
int LIBUSB_CALL libusb_open(libusb_device* dev, libusb_device_handle** dev_handle, int fd, LPCSTR szUsbPath);
void LIBUSB_CALL libusb_close(libusb_device_handle *dev_handle);
libusb_device * LIBUSB_CALL libusb_get_device(libusb_device_handle *dev_handle);
libusb_device * LIBUSB_CALL libusb_create(libusb_context *ctx, const char *szUsbPath);
...
int LIBUSB_CALL libusb_note_claimed_interface(libusb_device_handle* dev_handle, int interface_number);
int LIBUSB_CALL libusb_release_interface(libusb_device_handle *dev_handle,
	int interface_number);
int LIBUSB_CALL libusb_note_released_interface(libusb_device_handle *dev_handle, int interface_number);
