...
int linux_netlink_start_event_monitor(void)
{
	...
	if (linux_netlink_socket == -1) {
		usbi_err(NULL, "failed to create netlink socket (%d)", errno); // -rga: this triggers on Marshmallow
		goto err;
	}
	...
	/* TODO -- add authentication */ // saki
	// ret = setsockopt(linux_netlink_socket, SOL_SOCKET, SO_PASSCRED, &opt, sizeof(opt)); // saki
	if (ret == -1) {
...
{
	if (-1 == linux_netlink_socket) { // -rga AND saki
	...
	if (r <= 0)
		usbi_warn(NULL, "netlink control pipe signal failed; ignoring");
	...
	*devaddr  = 0;
	...
	if (!tmp) {
		...
		*detached = 1;
		...
		// pass through
	} else if (strcmp(tmp, "change") == 0) { // saki
		// pass through
	} else {
...
static void *linux_netlink_event_thread_main(void *arg)
{
	...
	while ((r = poll(fds, 2, -1)) >= 0 || errno == EINTR) {
		...
		if (fds[0].revents & POLLIN) {
			...
			nb = usbi_read(netlink_control_pipe[0], &dummy, sizeof(dummy));
			if (nb <= 0)
