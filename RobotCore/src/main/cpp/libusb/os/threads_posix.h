...
#if ENABLE_MUTEX_LOGGING // -rga 2018.05.08
    #define usbi_mutex_init(expr)		    (usbi_dbg("init %s=%p", #expr, expr),    usbi_mutex_init(expr))
    #define usbi_mutex_lock(expr)	        (usbi_dbg("lock %s=%p", #expr, expr),    usbi_mutex_lock(expr))
    #define usbi_mutex_unlock(expr)	        (usbi_dbg("unlock %s=%p", #expr, expr),  usbi_mutex_unlock(expr))
    #define usbi_mutex_trylock(expr)	    (usbi_dbg("trylock %s=%p", #expr, expr), usbi_mutex_trylock(expr))
    #define usbi_mutex_destroy(expr)        (usbi_dbg("destroy %s=%p", #expr, expr), usbi_mutex_destroy(expr))
#endif
