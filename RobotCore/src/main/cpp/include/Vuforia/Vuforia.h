...
enum INIT_ERRORCODE {
    ...
    INIT_EXTERNAL_PROVIDER_FAILED = -11             ///< External provider library failed to open / version number doesn't match
};
...
/// Sets up an external source via a named library which is expected to be on
/// the default path to be dynamically loaded. The module must support the Vuforia
/// external api
bool VUFORIA_API setExternalProviderLibrary(const char* libraryName, void* userData);
